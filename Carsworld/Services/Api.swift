...
class ApiService: NSObject {
    ...
    func fetchFromUrlString(urlString: String, completion: @escaping ([Voucher]) -> ()) {
            ...
            do {
                if let unwrappedData = data, let itemDictionaries = try JSONSerialization.jsonObject(with: unwrappedData, options: .mutableContainers) as? [[String: AnyObject ]] {
                    ...
                    let videoss = itemDictionaries.map({ (item) in
                        return Voucher(dictionary: item)
                    })
    ...
    func fetchVideos(completion: @escaping ([Voucher]) -> ()) {
    ...
    func fetchTrending(completion: @escaping ([Voucher]) -> ()) {
    ...
    func fetchDiscount(completion: @escaping ([Voucher]) -> ()) {
